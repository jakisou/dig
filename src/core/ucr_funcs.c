...
//#define US_LB_KEOGH_ANYWAY // this breaks the uniform scaling function's accuracy
...
//	printf(" "); // for reasons that remain unclear, putting this here prevents a realloc error
...
static inline void znormalize_prefix(data_t *ar, length_t m, length_t stopAfter) {
...
//TODO combine duplicate code here
...
data_t dtw(const data_t* A, const data_t* B, const data_t *cb, length_t m, length_t r, data_t bsf) {
	...
	//TODO calculate first column separately so we can get the check for (0,0)
	//out of the main loop
		// can also pull out check for inFirstCol
		// can also-er pull our check for inFirstRow, since this has to
		//  also be the first warp row
		// so really just have to check for firstWarpRow and newWarpRow
		// actually, can't pull out check for first row since could be before
		// warping path gets off of first row at start
    ...
    for (i = 0; i < m; i++) {	//for each column
        ...
        for(j = firstRowInWindow; j <= lastRowInWindow; j++, k++) {  //for each row in warping window
			...
			//With first column broken out (assuming we check for r = 0 and
			//just do an ED calculatation if this is the case elsewhere)
			//if inFirstWarpRow
				//parentCost = min(cost_prev[k],cost_prev[k+1]);
			//elseif inNewWarpRow || inFirstRow
				//parentCost = min(cost_prev[k],cost[k-1]);
			//else
				//parentCost = min(cost[k-1],min(cost_prev[k],cost_prev[k+1]));
			//end
			//cost[k] = parentCost + dist(A[i], B[j]);
...
//	array_print_with_name(q, m, "dtw q");
//	array_print_with_name(x, m, "dtw x");
//	printf("meanX, stdX = %.3f, %.3f\n", meanX, stdX);
...
//	array_print_with_name(q, m, "dtw qNorm");
...
//	array_print_with_name(uTmp, m, "u1");
//	array_print_with_name(lTmp, m, "l1");
...
//	array_print_with_name(q, m, "dtw xNorm");
...
//	printf("dtw meanX, stdX = %.3f, %.3f\n", mean, std);
//	printf("true meanX, stdX = %.3f, %.3f\n", array_mean(t,m), sqrt(array_variance(t,m)));
//	array_print_with_name(q, m, "dtw q");
//	array_print_with_name(t, m, "dtw x");
...
//	array_print_with_name(datazTmp, m, "dtw xNorm");
//	array_print_with_name(u_buff, m, "dtw uBuff");
//	array_print_with_name(l_buff, m, "dtw lBuff");
...
//			distance = calc_distance(t, tz, j, order, q, qo, uo, lo, cb1, cb2,
//									 l_buff, u_buff, mean, std, m, r, bsf);
			
//			distance = dtw_distance(query, t+j, tz, cb1, cb2, l_buff+I, u_buff+I,
//									 mean, std, bsf);
...
//	printf("building swm envelope with minLen, maxLen, warpR = %d, %d, %.3f\n",
//		   minLen, maxLen, r);
	
	// create the uniform scaling envelope for the query
	build_swm_envelope(sq->q, m,minLen,maxLen, r, l,u);
//	if (r < .001) {
//		printf("---------- SHOULD JUST BE A US ENVELOPE ----------\n");
//		data_t l2[maxLen];
//		data_t u2[maxLen];
//		build_unifscale_envelope(sq->q, m, minLen, maxLen, l2, u2);
//		array_print_with_name(l, minLen, "l");
//		array_print_with_name(l2, minLen, "l2");
//		array_print_with_name(u, minLen, "u");
//		array_print_with_name(u2, minLen, "u2");
//		if (!array_equal(l, l2, minLen) || !array_equal(u, u2, minLen)) {
//			printf("But it's not...\n");
//			exit(1);
//		}
//	} else {
//		printf("---------- SHOULD NOT JUST BE A US ENVELOPE ----------\n");
//		printf("r, m, minLen, maxLen = %.3f, %d, %d, %d\n", r, m, minLen, maxLen);
//		printf("------------------------------------------------------\n");
//	}
	
//	if (m == minLen) {
//		if (!array_equal(sq->q, l, minLen) || !array_equal(sq->q, u, minLen)) {
//			printf("ERROR: build_swm_envelop: 0 warp didn't yield the right env\n");
//			array_print_with_name(sq->q, minLen, "q");
//			array_print_with_name(u, minLen, "u");
//			array_print_with_name(l, minLen, "l");
//		}
//	}
...
//		data_t stdQ = sqrt(array_variance(s,len));
//		if (stdQ == 0) {
//			printf("std of 0 for len = %d\n", len);
//			exit(1);
//		}
...
//	printf("freeing swm query with minLen, maxLen = %d, %d\n",
//		   query->minLen, query->maxLen);
...
//	printf("swm_test_all_scalings received r, m, minLen %.3f, %d, %d\n", r,m,minLen);
//	printf("rStepsMin as data_t = %.3f\n", r * minLen);
//	rSteps = r*minLen;
//	printf("rStepsMin as int = %d\n", rSteps);
...
//	printf("meanY,stdY, meanX,stdX = %.3f,%.3f, %.3f,%.3f\n",
//		   meanY, stdY, meanX, stdX);
	
	rSteps = round(r * minLen);
//	printf("now has r, m, minLen %.3f, %d, %d\n", r,m,minLen);
//	printf("new rStepsMin as int = %d\n", rSteps);
//	dist = euclidean_dist_fixed_order(yTmp, dataStart, minLen, meanY, stdY, meanX, stdX, bsf);
...
//	array_print_with_name(query, m, "q");
//	array_print_with_name(yTmp, minLen, "yMinLen");
...
//		array_resample(query, TEST, m, len);
//		assert(array_equal(yTmp, TEST, len));
		meanY = sumY / len;
		stdY = sqrt(sumY2 / len - meanY * meanY);
		if (stdY<=0) stdY = DBL_MIN;
		rSteps = round(r * len);
//		array_print_with_name(yTmp, len, "yTmp");
//		array_print_with_name(dataStart, len, "x");
//		array_copy(dataStart, TEST, len);
//		znormalize(TEST, len);
//		array_print_with_name(TEST, len, "xNorm");
		
//		data_t trueMeanX = array_mean(dataStart, len);
//		data_t trueStdX = sqrt(array_variance(dataStart, len));
//		if (trueMeanX != meanX || trueStdX != stdX) {
//			printf("ERROR: swm_test_all_scalings: x stats wrong\n");
//			printf("trueMeanX, trueStdX = %.3f, %.3f\n", trueMeanX, trueStdX);
//			printf("meanX, stdX = %.3f, %.3f\n", meanX, stdX);
//			exit(1);
//		}

		
//		dist = euclidean_dist_fixed_order(yTmp, dataStart, len, meanY, stdY, meanX, stdX, bsf);
		dist = dtw_distance_from_scratch(yTmp, dataStart,
...
data_t swm_distance_and_len_envelope(const swm_query *query, const data_t *data,
									 ...
									 length_t* bestLen) {
//	*bestLen =  query->m;
//	return dtw_distance(query->dtwQueries[0], data, l_buff, u_buff, meanX, stdX, bsf);
	length_t maxLen = query->maxLen;
	length_t minLen = query->minLen;
	
//	short int shouldBeEquivalent = 0;
//	data_t lb_k_us;
	
	if (useEnvelope) {
		const data_t* dataCompareStart = data + maxLen - minLen;
		
		//Yep, these are exactly identical
//		if (query-> r < .001 && ((query->m - minLen) == (maxLen - query->m) ) ) {
//			shouldBeEquivalent = 1;
//			
//			printf("---------- SHOULD BE EQUIVALENT TO US QUERY ----------\n");
//			us_query * usq = us_query_new(query->q, query->m, query->minLen, query->maxLen);
//			if (! array_equal(usq->lo, query->lo, minLen) ||
//				! array_equal(usq->uo, query->uo, minLen) ||
//				! array_equal(usq->q, query->q, query->m) ||
//				! intarray_equal(usq->envOrder, query->envOrder, minLen)) {
//				printf("BUT IS NOT...\n");
//				printf("us: m, minLen, maxLen = %d, %d, %d\n",
//					   usq->m, usq->minLen, usq->maxLen);
//				printf("swm: m, minLen, maxLen = %d, %d, %d,  r=%.3f\n",
//					   query->m, query->minLen, query->maxLen, query->r);
//				printf("\n");
//				array_print_with_name(usq->lo, minLen, "us_lo");
//				array_print_with_name(query->lo, minLen, "swm_lo");
//				printf("\n");
//				array_print_with_name(usq->uo, minLen, "us_uo");
//				array_print_with_name(query->uo, minLen, "swm_uo");
//				printf("\n");
//				array_print_with_name(usq->q, minLen, "us_q");
//				array_print_with_name(query->q, minLen, "swm_q");
//				printf("\n");
//				intarray_print_with_name(usq->envOrder, minLen, "us_envOrder");
//				intarray_print_with_name(query->envOrder, minLen, "swm_envOrder");
//				
//				exit(1);
//			}
//			lb_k_us = lb_keogh_us_query(usq->envOrder, dataCompareStart, usq->uo, usq->lo, usq->minLen, meanX, stdX, bsf);
//			
//			
//		}
...
//		if (shouldBeEquivalent && lb_k != lb_k_us) {
//			printf("lbk, lbk_us = %.3f, %.3f....wat!?\n", lb_k, lb_k_us);
//			exit(1);
//		}
