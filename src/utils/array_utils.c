...
data_t** array_reshape(const data_t* data, unsigned int len, unsigned int newNumDims) {
	...
	if ( newArraysLen * newNumDims != len) {
		printf("array_reshape: newNumDims %d is not factor of array length %d", newNumDims, len);
		return NULL;
	}
	
	int i,j,readFrom=0;
	//initialize each array ptr and the array the containing them; note
	//that the arrays are allocated as one contiguous block of memory
	data_t** arrays = malloc(newNumDims*sizeof(data_t*));
	data_t* arrayContents = malloc(len*sizeof(double));
	for (i = 0; i < newNumDims; i++) {
		arrays[i] = arrayContents + i*newArraysLen;
	}
	
	//copy the values from the 1D array to be reshaped
	for (i = 0; i < len; i+=newNumDims) {
		for (j = 0; j < newNumDims; j++, readFrom++) {
			arrays[j][i] = data[readFrom];
		}
	}
